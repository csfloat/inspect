...
class Postgres {
  ...
  async ensureSchema() {
    await this.pool.query(`CREATE TABLE IF NOT EXISTS raw_assets (
            ms          bigint  NOT NULL,
            a           bigint  NOT NULL,
            d           bigint  NOT NULL,
        ...
        ALTER TABLE raw_assets ADD COLUMN IF NOT EXISTS floatid BIGINT;
        ALTER TABLE raw_assets ADD COLUMN IF NOT EXISTS price INTEGER;
        ALTER TABLE raw_assets ADD COLUMN IF NOT EXISTS listed_price INTEGER;
        ALTER TABLE raw_assets ADD COLUMN IF NOT EXISTS tradable_after TIMESTAMP;
    ...
    await this.pool
      .query(`CREATE INDEX IF NOT EXISTS i_stickers ON raw_assets USING gin (stickers jsonb_path_ops) 
                                WHERE stickers IS NOT NULL`);
    await this.pool.query(`CREATE INDEX IF NOT EXISTS i_paintwear ON raw_assets (paintwear)`);
    await this.pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS i_unique_item ON 
                                raw_assets (defindex, paintindex, paintwear, paintseed)`);
    await this.pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS i_unique_fid ON raw_assets (floatid)`);
  }
  ...
  async handleBulkInsert(data) {
    winston.debug(`Start handleBulkInsert ${JSON.stringify(data)}`);
    const values = [];
    ...
    for (let [item, price] of data) {
      ...
      let tradable_after =  item.tradable_after instanceof Date ? item.tradable_after.toISOString() : null;
    ...
    winston.debug(`Data length: ${values.length}`);
    if (values.length === 0) {
      winston.debug('No data to insert');
      return;
    }

    winston.debug(`Try query ${JSON.stringify(data)}`);
    try {
  ...
  static buildQuery(itemCount) {
    ...
    for (let c = 0; c < itemCount; c++) {
      values.push(
        `($${i++}, $${i++}, $${i++}, $${i++}, $${i++}, $${i++}, $${i++}, $${i++}, $${i++}, $${i++}, $${i++}::jsonb, now(), $${i++}, NULL, $${i++}, $${i++}::timestamp)`,
      );
    }

    return `INSERT INTO raw_assets (ms, a, d, paintseed, paintwear, defindex, paintindex, stattrak, souvenir, props, stickers, updated, rarity, floatid, price, tradable_after)
                VALUES ${values.join(", ")} 
                ON CONFLICT(defindex, paintindex, paintwear, paintseed) 
                DO UPDATE SET ms=excluded.ms, a=excluded.a, d=excluded.d, stickers=excluded.stickers, tradable_after=excluded.tradable_after, updated=now()`;
  }

  updateItemPrice(assetId, price) {
    return this.pool.query(`UPDATE raw_assets SET price = $1 WHERE a = $2`, [price, assetId]);
  }
