...
class Postgres {
  ...
  async ensureSchema() {
    await this.pool.query(`CREATE TABLE IF NOT EXISTS raw_items (
            ms          bigint  NOT NULL,
            a           bigint  NOT NULL,
            d           bigint  NOT NULL,
        ...
        ALTER TABLE raw_items ADD COLUMN IF NOT EXISTS floatid BIGINT;
        ALTER TABLE raw_items ADD COLUMN IF NOT EXISTS price INTEGER;
        ALTER TABLE raw_items ADD COLUMN IF NOT EXISTS listed_price INTEGER;
    ...
    await this.pool
      .query(`CREATE INDEX IF NOT EXISTS i_stickers ON raw_items USING gin (stickers jsonb_path_ops) 
                                WHERE stickers IS NOT NULL`);
    await this.pool.query(`CREATE INDEX IF NOT EXISTS i_paintwear ON raw_items (paintwear)`);
    await this.pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS i_unique_item ON 
                                raw_items (defindex, paintindex, paintwear, paintseed)`);
    await this.pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS i_unique_fid ON raw_items (floatid)`);
  }
  ...
  static buildQuery(itemCount) {
    ...
    return `INSERT INTO raw_items (ms, a, d, paintseed, paintwear, defindex, paintindex, stattrak, souvenir, props, stickers, updated, rarity, floatid, price)
  ...
  updateItemPrice(assetId, price) {
    return this.pool.query(`UPDATE raw_items SET price = $1 WHERE a = $2`, [price, assetId]);
  }
