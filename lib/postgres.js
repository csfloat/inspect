...
class Postgres {
    ...
    async handleBulkInsert(data) {
        ...
        for (let [item, price] of data) {
            ...
            values.push(format('(%L, %L, %L, %L, %L, %L, %L, %L, %L, %L, %L::jsonb, now(), %L, NULL, %L)',
        ...
        try {
            const query = format(`INSERT INTO items VALUES %s ON CONFLICT(defindex, paintindex, paintwear, paintseed) 
                DO UPDATE SET ms=excluded.ms, a=excluded.a, d=excluded.d, stickers=excluded.stickers, updated=now()`, values.join(','));
