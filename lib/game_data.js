...
class GameData {
  ...
  /*
    ...
    */
  addAdditionalItemProperties(iteminfo, steamClient) {
    ...
    winston.debug(`steamClient: ${JSON.stringify(steamClient)})}`);

    const cdn = new csgoCDN(steamClient, cdnOpts)
    ...
    for (const sticker of iteminfo.stickers || []) {
      ...
      sticker.imageurl = cdn.getStickerURL(sticker.name, false);
    }
