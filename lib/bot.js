...
class Bot extends EventEmitter {
    ...
    constructor(settings) {
        ...
        this.steam = new SteamUser(Object.assign({
    ...
    reLogIn() {
        // Ensure there is a wait between relogin attempts
    ...
    bindEventHandlers() {
        ...
        this.steamClient.once('disconnected', (eresult, msg) => {
            winston.warn(`${this.username} Logged off, reconnecting! (${eresult}, ${msg})`);

            this.reLogIn();
        });

        this.steamClient.once('loggedOn', (details, parental) => {
            winston.info(`${this.username} Log on OK`);

            // Launch CS:GO GC Connection
    ...
    sendFloatRequest(data) {
        return new Promise((resolve, reject) => {
            ...
            if (!this.ready) {
                ...
                this.csgoClient.inspectItem(data.s ? data.s : data.m, data.a, data.d);
            }
