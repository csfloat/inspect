...
class Bot extends EventEmitter {
    ...
    bindEventHandlers() {
        this.steamClient.once('connected', () => {
            ...
            winston.info(`${this.username} Connected`);
        ...
        this.steamClient.once('loggedOff', () => {
            winston.warn(`${this.username} Logged off, reconnecting!`);

            this.reLogIn();
        });

        this.steamClient.once('servers', (servers) => {
            winston.debug(`${this.username} Received servers.`);
            fs.writeFileSync('servers.json', JSON.stringify(servers, null, 2));
        });

        this.steamClient.once('sentry', (sentry) => {
            winston.debug(`${this.username} Received sentry.`);
        ...
        this.steamClient.once('logOnResponse', (response) => {
            if (response.eresult == this.Steam.EResult.OK) {
                ...
                winston.error(`Error logging in ${this.username}:`, response);
                ...
                if (response.eresult && login_error_msgs[response.eresult] != undefined) {
                    winston.error(this.username + ': ' + login_error_msgs[response.eresult]);
                }
            }
        });
        ...
        this.csgoClient.on('unready', () => {
            winston.warn(`${this.username} CSGO unready, trying to reconnect!`);

            this.reLogIn();
        });

        this.csgoClient.on('unhandled', (msg) => {
            winston.warn(`${this.username} Unhandled msg: ${msg}`);
        });
    }
