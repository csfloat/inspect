...
class Bot extends EventEmitter {
  ...
  constructor(settings, proxy, username, password, auth) {
    super();

    if (typeof proxy === "string") {
      if (proxy.startsWith("http://")) {
        this.httpProxy = proxy.replace("http://", "");
      } else if (proxy.startsWith("socks5://")) {
        this.socksProxy = proxy.replace("socks5://", "");
      }
    }
    ...
    console.log(this.httpProxy);

    this.steamClient = new SteamUser(
      Object.assign(
        {
          httpProxy: this.httpProxy ? this.httpProxy : undefined,
          socksProxy: this.socksProxy ? this.socksProxy : undefined,
  ...
  logOff() {
    return this.steamClient.logOff()
  }
  ...
  bindEventHandlers() {
    ...
    this.steamClient.on("steamGuard", (domain, callback, lastCodeWrong) =>{
      winston.debug("Steam Guard code needed from email ending in " + domain);
    });
    ...
    this.csgoClient.on("inspectItemInfo", itemData => {
      if (this.resolve && this.currentRequest) {
        ...
        if (Array.isArray(this.csgoClient.inventory)) {
          let inventoryItem = this.csgoClient.inventory.find(it => it.paint_index === itemData.iteminfo.paintindex &&
                                                                   it.paint_wear === itemData.iteminfo.paintwear &&
                                                                   it.paint_seed === itemData.iteminfo.paintseed
          );

          if (inventoryItem) itemData.iteminfo.tradable_after = inventoryItem.tradable_after;
        }
